...
function makeDiningBayInlay(x, z) {
...
function makeChair(x, z, rotationY, name) {
  const group = new THREE.Group();
  group.name = name;
  group.position.set(x, 0, z);
...
function makePlaceSetting(parent, x, z, rotationY) {
...
function makeTableSet(x, z, label) {
  const group = new THREE.Group();
  group.name = label;
  group.position.set(x, 0, z);
...
// Four original lighting positions remain; only the rear-left dining set is absent.
for (const [x, z] of [
  [-3.28, 3.15],
  [3.32, 3.15],
  [-3.28, -3.65],
...
window.__SCENE_METADATA__ = {
  ...
  frames: FRAME_COUNT,
  duration: DURATION,
  removedDiningSet: 'left_second_table',
  remainingDiningSets: [
    'left_first_table',
    'right_first_table',
    'right_second_table',
