...
function hud(s) { window.__bfTrace?.add(16);
  ctx.save();
...
function seek(t){ window.__bfTrace?.add(47);
...
function pause(){ window.__bfTrace?.add(55);playing=false;audio.pause();}
function play(){ window.__bfTrace?.add(56);
  if(time>=5)seek(0);
  audio.currentTime=time;audio.play().catch(()=>{});
  clockBase=performance.now()-time*1000;playing=true;
}
function animate(now){ window.__bfTrace?.add(61);
...
function diagnostics(){ window.__bfTrace?.add(68);
