...
function material(color, roughness = 0.7, metalness = 0) { window.__bfTrace?.add(30);
...
function box(name, size, position, mat, parent = scene, cast = true, receive = true) { window.__bfTrace?.add(51);
...
function cylinder(name, radiusTop, radiusBottom, height, segments, position, mat, parent = scene) { window.__bfTrace?.add(61);
...
function roundedShape(width, height, radius) { window.__bfTrace?.add(74);
...
function roundedPanel(name, width, height, depth, radius, position, mat, parent = scene) { window.__bfTrace?.add(90);
...
function leafGeometry(length = 0.72, width = 0.25) { window.__bfTrace?.add(109);
  const shape = new THREE.Shape();
  shape.moveTo(0, 0);
...
function makeRoom() { window.__bfTrace?.add(119);
...
function makeSofa() { window.__bfTrace?.add(223);
...
function makeRugAndTable() { window.__bfTrace?.add(260);
...
function makeChairAndLamp() { window.__bfTrace?.add(306);
...
function makeForegroundPlant() { window.__bfTrace?.add(346);
...
function makeDecor() { window.__bfTrace?.add(383);
...
function smootherStep(value) { window.__bfTrace?.add(477);
  const x = THREE.MathUtils.clamp(value, 0, 1);
  return x * x * x * (x * (x * 6 - 15) + 10);
}

function updateScene(time) { window.__bfTrace?.add(482);
...
function render() { window.__bfTrace?.add(501);
  renderer.render(scene, camera);
}

function animate() { window.__bfTrace?.add(505);
  if (playing) {
