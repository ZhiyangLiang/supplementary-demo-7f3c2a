...
function cylinder(radiusTop, radiusBottom, height, material, position, parent = scene, segments = 20) {
  const object = mesh(new THREE.CylinderGeometry(radiusTop, radiusBottom, height, segments), material, parent);
  object.position.set(...position);
  return object;
}

function roundedPanel(width, height, depth, radius, material, parent) {
...
function addLaneStripe(x, z, width, length, material = mats.paintWhite) {
...
function createGatePanel(parent, direction) {
...
function fenceSection(x, z, length) {
  const group = new THREE.Group();
  group.position.set(x, 0, z);
  scene.add(group);
...
function createHumanoidLeg(parent, side, materials) {
...
function createHumanoidArm(parent, side, materials) {
...
function updateCamera(z, ground) {
