...
function makeStoneTexture() { window.__bfTrace?.add(35);
...
function meshBox(name, size, position, material, castShadow = true, receiveShadow = true) { window.__bfTrace?.add(166);
...
function addRailSegment(name, size, position) { window.__bfTrace?.add(176);
...
function addLevel(level, y) { window.__bfTrace?.add(203);
  const slabParts = [
...
function makeHelix(phase, radius, height, turns, material, tubeRadius) { window.__bfTrace?.add(330);
  const points = [];
...
function addBench(x, z, rotation = 0) { window.__bfTrace?.add(392);
...
function addPlanter(x, y, z, scale = 1) { window.__bfTrace?.add(414);
...
const people = [];
function addPerson(x, y, z, color, rotation = 0) { window.__bfTrace?.add(452);
...
function smoothstep(edge0, edge1, value) { window.__bfTrace?.add(551);
  const p = THREE.MathUtils.clamp((value - edge0) / (edge1 - edge0), 0, 1);
  return p * p * (3 - 2 * p);
}

function setCamera(time) { window.__bfTrace?.add(556);
...
function updateScene(time) { window.__bfTrace?.add(574);
  setCamera(time);
...
function renderAt(time) { window.__bfTrace?.add(598);
  currentTime = THREE.MathUtils.clamp(Number(time) || 0, 0, DURATION);
  updateScene(currentTime);
  renderer.render(scene, camera);
}

function tick(now) { window.__bfTrace?.add(604);
  if (!playing) {
    return;
  }
...
function cameraState() { window.__bfTrace?.add(613);
  return {
