...
const materials = {
  ...
  robot: matte(0xb8c6ca, 0.24, 0.76),
  robotDark: matte(0x111a1f, 0.22, 0.84),
...
function createHumanoidRobot() { window.__bfTrace?.add(354);
  ...
  pelvis.add(box(0.42, 0.28, 0.62, materials.robot));
  pelvis.add(box(0.46, 0.12, 0.5, materials.robotDark, -0.03, 0.12, 0));
  pelvis.add(box(0.05, 0.13, 0.28, materials.blue, 0.225, 0.02, 0));
  pelvis.add(box(0.025, 0.08, 0.28, materials.blue, -0.225, 0.02, 0));
  body.add(pelvis);

  const torso = new THREE.Group();
  torso.position.y = 1.55;
  torso.add(box(0.48, 0.66, 0.9, materials.robot));
  torso.add(box(0.08, 0.48, 0.58, materials.robotDark, 0.265, 0.02, 0));
  torso.add(box(0.035, 0.32, 0.36, materials.blue, 0.315, 0.05, 0));
  torso.add(box(0.36, 0.12, 0.72, materials.robotDark, -0.12, -0.31, 0));
  torso.add(box(0.025, 0.34, 0.1, materials.blue, -0.252, 0.04, 0));
  for (const side of [-1, 1]) {
    const shoulderShell = mesh(new THREE.SphereGeometry(0.22, 20, 14), materials.robot);
    shoulderShell.scale.set(0.92, 0.9, 1.08);
    shoulderShell.position.set(0, 0.23, side * 0.49);
  ...
  head.add(box(0.46, 0.46, 0.42, materials.robot, 0, 0, 0));
  head.add(box(0.055, 0.24, 0.33, materials.robotDark, 0.255, 0.015, 0));
  head.add(box(0.025, 0.1, 0.25, materials.blue, 0.288, 0.045, 0));
  const crown = cylinder(0.115, 0.07, materials.robotDark, 20);
  crown.position.y = 0.245;
  head.add(crown);
  const crownBand = cylinder(0.12, 0.025, materials.blue, 20);
  crownBand.position.y = 0.27;
  head.add(crownBand);
  body.add(head);

  const neck = cylinder(0.1, 0.22, materials.joint, 18);
  neck.position.y = 1.94;
  body.add(neck);

  const legs = [];
  const upperLegLength = 0.48;
  const lowerLegLength = 0.49;
  for (const side of [-1, 1]) {
    const hip = new THREE.Group();
    hip.position.set(0, 1.06, side * 0.23);
    const hipMotor = cylinder(0.135, 0.17, materials.joint, 18);
    hipMotor.rotation.x = Math.PI / 2;
    hip.add(hipMotor);

    const upper = mesh(
      new THREE.CylinderGeometry(0.095, 0.125, upperLegLength, 16),
      materials.robotDark,
    );
    upper.position.y = -upperLegLength / 2;
    hip.add(upper);
    hip.add(box(0.15, 0.3, 0.18, materials.robot, 0.02, -0.22, 0));

    const knee = new THREE.Group();
    knee.position.y = -upperLegLength;
    const kneeMotor = cylinder(0.12, 0.16, materials.joint, 18);
    kneeMotor.rotation.x = Math.PI / 2;
    knee.add(kneeMotor);

    const lower = mesh(
      new THREE.CylinderGeometry(0.08, 0.105, lowerLegLength, 16),
      materials.robotDark,
    );
    lower.position.y = -lowerLegLength / 2;
    knee.add(lower);
    knee.add(box(0.13, 0.31, 0.16, materials.robot, -0.015, -0.23, 0));

    const ankle = new THREE.Group();
    ankle.position.y = -lowerLegLength;
    const ankleMotor = cylinder(0.085, 0.13, materials.joint, 16);
    ankleMotor.rotation.x = Math.PI / 2;
    ankle.add(ankleMotor);
    ankle.add(box(0.34, 0.11, 0.23, materials.rubber, 0.095, -0.105, 0));
    ankle.add(box(0.28, 0.08, 0.2, materials.robot, 0.06, -0.04, 0));
    knee.add(ankle);
    hip.add(knee);
    body.add(hip);
    legs.push({ hip, knee, ankle, side, phaseOffset: side < 0 ? 0 : Math.PI });
  }
  ...
  for (const side of [-1, 1]) {
    ...
    shoulder.position.set(0, 1.78, side * 0.51);
    const shoulderMotor = cylinder(0.13, 0.15, materials.joint, 18);
    shoulderMotor.rotation.x = Math.PI / 2;
    shoulder.add(shoulderMotor);

    const upperArm = mesh(
      new THREE.CylinderGeometry(0.075, 0.1, upperArmLength, 16),
      materials.robotDark,
    );
    upperArm.position.y = -upperArmLength / 2;
    shoulder.add(upperArm);
    shoulder.add(box(0.13, 0.24, 0.16, materials.robot, 0, -0.19, 0));

    const elbow = new THREE.Group();
    elbow.position.y = -upperArmLength;
    const elbowMotor = cylinder(0.095, 0.13, materials.joint, 16);
    elbowMotor.rotation.x = Math.PI / 2;
    elbow.add(elbowMotor);

    const forearm = mesh(
      new THREE.CylinderGeometry(0.065, 0.085, lowerArmLength, 14),
      materials.robotDark,
    );
    forearm.position.y = -lowerArmLength / 2;
    elbow.add(forearm);
    elbow.add(box(0.12, 0.25, 0.15, materials.robot, 0.01, -0.19, 0));
    elbow.add(box(0.16, 0.15, 0.18, materials.robotDark, 0.025, -0.43, 0));
    shoulder.add(elbow);
    body.add(shoulder);
    arms.push({ shoulder, elbow, side, phaseOffset: side < 0 ? 0 : Math.PI });
  }

  scene.add(root);
  return { root, body, pelvis, torso, head, legs, arms };
}
...
function travelState(time) { window.__bfTrace?.add(521);
...
function updateScene(time) { window.__bfTrace?.add(545);
  ...
  robot.body.position.y = 0.015 + Math.abs(Math.sin(gait)) * 0.032 * movementBlend;
  robot.body.rotation.x = Math.sin(gait) * 0.018 * movementBlend;
  robot.body.rotation.z = -0.035 * movementBlend;
  robot.pelvis.rotation.x = -Math.sin(gait) * 0.035 * movementBlend;
  robot.pelvis.rotation.y = Math.sin(gait) * 0.04 * movementBlend;
  robot.torso.rotation.x = -Math.sin(gait) * 0.022 * movementBlend;
  robot.torso.rotation.y = -Math.sin(gait) * 0.025 * movementBlend;
  robot.head.rotation.x = -Math.sin(gait) * 0.015 * movementBlend;
  robot.head.rotation.z = 0.018 * movementBlend;

  for (const leg of robot.legs) {
    const phase = gait + leg.phaseOffset;
    const stride = Math.sin(phase) * movementBlend;
    const lift = Math.max(0, Math.sin(phase + 0.28)) * movementBlend;
    leg.hip.rotation.z = stride * 0.42;
    leg.hip.rotation.x = leg.side * (0.045 + 0.018 * movementBlend);
    leg.knee.rotation.z = -lift * 0.72;
    leg.ankle.rotation.z = -(leg.hip.rotation.z + leg.knee.rotation.z) * 0.78;
  }

  for (const arm of robot.arms) {
    const phase = gait + arm.phaseOffset;
    const swing = Math.sin(phase) * movementBlend;
    arm.shoulder.rotation.z = -swing * 0.48;
    arm.shoulder.rotation.x = arm.side * 0.08;
    arm.elbow.rotation.z = -0.14 - Math.max(0, swing) * 0.34;
  }
...
function animate() { window.__bfTrace?.add(624);
  if (!paused) {
