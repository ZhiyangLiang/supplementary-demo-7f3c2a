...
function addHorseMesh(geometry, material, position, scale, rotation = [0, 0, 0]) { window.__bfTrace?.add(332);
...
function alignSegment(mesh, start, end) { window.__bfTrace?.add(522);
...
function gaitFoot(definition, time) { window.__bfTrace?.add(530);
...
function updateLeg(leg, time, bob, pitch) { window.__bfTrace?.add(549);
...
function updateTail(time, bob) { window.__bfTrace?.add(603);
...
function horseXAt(time) { window.__bfTrace?.add(628);
  return -5.55 + time * 2.56;
}

function updateSplash(system, leg, legIndex, time) { window.__bfTrace?.add(632);
...
function updateWater(time) { window.__bfTrace?.add(671);
...
function updateCamera(time, horseWorldX) { window.__bfTrace?.add(690);
...
function updateScene(time) { window.__bfTrace?.add(705);
...
function animate() { window.__bfTrace?.add(750);
  if (playing) {
