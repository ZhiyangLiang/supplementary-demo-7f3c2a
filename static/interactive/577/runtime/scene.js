...
function buildRouteGuide() { window.__bfTrace?.add(289);
...
function createRobot() { window.__bfTrace?.add(326);
...
function clamp01(value) { window.__bfTrace?.add(408);
  return Math.max(0, Math.min(1, value));
}

function smootherstep(value) { window.__bfTrace?.add(412);
  const x = clamp01(value);
  return x * x * x * (x * (x * 6 - 15) + 10);
}

function updateAt(time) { window.__bfTrace?.add(417);
...
function pause() { window.__bfTrace?.add(458);
  playing = false;
  if (animationFrame) {
...
function tick() { window.__bfTrace?.add(466);
  if (!playing) return;
  updateAt((currentTime + clock.getDelta()) % DURATION);
  animationFrame = requestAnimationFrame(tick);
}

function play() { window.__bfTrace?.add(472);
  if (playing) return;
  playing = true;
  clock.getDelta();
  animationFrame = requestAnimationFrame(tick);
...
function seek(seconds) { window.__bfTrace?.add(479);
  pause();
  updateAt(Number(seconds));
}

function getCameraState() { window.__bfTrace?.add(484);
  return {
