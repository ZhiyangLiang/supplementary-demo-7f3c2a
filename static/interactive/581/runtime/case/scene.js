...
function segment(material, radius, parent) { window.__bfTrace?.add(386);
  return mesh(new THREE.CylinderGeometry(radius, radius * 0.9, 1, 12), material, parent);
}

function alignSegment(object, start, end) { window.__bfTrace?.add(390);
...
function clamp01(value) { window.__bfTrace?.add(428);
  return Math.max(0, Math.min(1, value));
}

function smoothstep(value) { window.__bfTrace?.add(432);
  return value * value * (3 - 2 * value);
}

function travelProgress(time) { window.__bfTrace?.add(436);
...
function updateRobot(time) { window.__bfTrace?.add(450);
...
function updateCamera(time) { window.__bfTrace?.add(503);
...
function renderAt(time) { window.__bfTrace?.add(522);
...
function animate() { window.__bfTrace?.add(529);
  if (!paused) {
