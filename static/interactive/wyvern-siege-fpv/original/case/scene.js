...
function box(x,y,z,w,h,d,m,parent=scene,name='') { return mesh(boxG,m,V(x,y,z),V(w,h,d),parent,name); }
function ball(x,y,z,rx,ry,rz,m,parent=scene,name='') { return mesh(sphereG,m,V(x,y,z),V(rx,ry,rz),parent,name); }
function rod(a,b,r,m,parent=scene,sides=6) {
...
function cone(x,y,z,r,h,m,parent=scene) {return mesh(new THREE.ConeGeometry(r,h,7),m,V(x,y,z),V(1,1,1),parent);}
function lineCurve(points,r,m,parent=scene) {
  return mesh(new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points),24,r,6,false),m,V(),V(1,1,1),parent);
}
function crenels(cx,y,z,width,parent=scene) {
...
function house(x,z,w,d,h,index){
...
function soldier(x,y,z,defender=false){
...
function updateFlame(mesh,start,end,t,factor){
...
function sweepX(t){return mix(-8.8,9.2,smooth((t-5.15)/2.6));}
function contactTime(x){
  let lo=5.15,hi=7.75;
