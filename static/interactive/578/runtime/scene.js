...
function addMesh(geometry, material, position, parent = scene, cast = true, receive = true) { window.__bfTrace?.add(69);
...
function addBox(name, size, position, material, parent = scene, cast = true, receive = true) { window.__bfTrace?.add(78);
  const mesh = addMesh(new THREE.BoxGeometry(...size), material, position, parent, cast, receive);
  mesh.name = name;
  return mesh;
}

function textTexture(text, foreground, background, width = 1024, height = 256) { window.__bfTrace?.add(84);
...
function addSign(text, size, position, rotation, foreground = '#eefafa', background = '#247c7a') { window.__bfTrace?.add(102);
...
function addPlant(position, scale = 1, parent = scene) { window.__bfTrace?.add(112);
...
function createPerson(position, colors, seated = false, rotationY = 0) { window.__bfTrace?.add(134);
...
function buildArchitecture() { window.__bfTrace?.add(161);
...
function buildSeatingIsland() { window.__bfTrace?.add(245);
...
const route = new THREE.CatmullRomCurve3([
  new THREE.Vector3(5.8, 0.06, 10.45),
  new THREE.Vector3(6.1, 0.06, 8.35),
  new THREE.Vector3(6.65, 0.06, 5.9),
  new THREE.Vector3(6.3, 0.06, 3.65),
  new THREE.Vector3(6.35, 0.06, 0.35),
  new THREE.Vector3(6.15, 0.06, -3.0),
  new THREE.Vector3(6.0, 0.06, -6.25),
  new THREE.Vector3(4.6, 0.06, -8.65),
  new THREE.Vector3(1.15, 0.06, -9.55),
  new THREE.Vector3(-1.1, 0.06, -9.2),
  new THREE.Vector3(-2.28, 0.06, -10.85),
], false, 'centripetal', 0.42);

let goalBeacon;

function buildRouteGuide() { window.__bfTrace?.add(291);
...
function createRobot() { window.__bfTrace?.add(328);
...
function clamp01(value) { window.__bfTrace?.add(410);
  return Math.max(0, Math.min(1, value));
}

function smootherstep(value) { window.__bfTrace?.add(414);
  const x = clamp01(value);
  return x * x * x * (x * (x * 6 - 15) + 10);
}

function updateAt(time) { window.__bfTrace?.add(419);
...
function pause() { window.__bfTrace?.add(460);
  playing = false;
  if (animationFrame) {
...
function tick() { window.__bfTrace?.add(468);
  if (!playing) return;
  updateAt((currentTime + clock.getDelta()) % DURATION);
  animationFrame = requestAnimationFrame(tick);
}

function play() { window.__bfTrace?.add(474);
  if (playing) return;
  playing = true;
  clock.getDelta();
  animationFrame = requestAnimationFrame(tick);
...
function seek(seconds) { window.__bfTrace?.add(481);
  pause();
  updateAt(Number(seconds));
}

function getCameraState() { window.__bfTrace?.add(486);
  return {
