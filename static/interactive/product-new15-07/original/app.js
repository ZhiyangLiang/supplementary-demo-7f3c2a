...
function mesh(g,m,x=0,y=0,z=0){let o=new T.Mesh(g,m);o.position.set(x,y,z);o.castShadow=true;o.receiveShadow=true;scene.add(o);return o}
function oval(m,x,y,z,sx,sy,sz){const o=mesh(new T.SphereGeometry(1,48,24),m,x,y,z);o.scale.set(sx,sy,sz);return o}
function cyl(m,x,y,z,r,h){return mesh(new T.CylinderGeometry(r,r,h,48),m,x,y,z)}
function bar(a,b,r,m){const d=new T.Vector3().subVectors(b,a),o=mesh(new T.CylinderGeometry(r,r,d.length(),24),m);o.position.copy(a).add(b).multiplyScalar(.5);o.quaternion.setFromUnitVectors(new T.Vector3(0,1,0),d.normalize());return o}
