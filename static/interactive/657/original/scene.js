...
function cylinder(name, radiusTop, radiusBottom, height, position, material, options = {}) {
...
function beamBetween(name, start, end, thickness, material = mats.steel) {
...
function addIBeam(name, position, length, orientation = 'x', scale = 1) {
...
const pendantGroups = [];
function addPendant(x, z, cableTop, lampY, size = 1) {
  const group = new THREE.Group();
...
function addSofa(x, z, rotation = 0, material = mats.leather, elevation = 0) {
  const group = new THREE.Group();
  group.name = `sofa ${x} ${z}`;
...
function addRoundTable(x, z, radius = 0.75, height = 0.48, elevation = 0) {
