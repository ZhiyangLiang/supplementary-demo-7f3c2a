...
function mat(color, roughness = 0.65, metalness = 0.05, extra = {}) { window.__bfTrace?.add(37);
...
function mesh(geometry, material, parent = scene) { window.__bfTrace?.add(46);
...
function addBox(size, position, material, parent = scene) { window.__bfTrace?.add(78);
...
function addPedestalPlaque(group, index) { window.__bfTrace?.add(160);
  const plaque = addBox(
    [0.54, 0.2, 0.045],
    [0, 0.56, 0.686],
...
function addSculpture(parent, spec) { window.__bfTrace?.add(178);
...
const cameraRouteCurve = new THREE.CatmullRomCurve3(
  cameraRoutePoints,
  false,
  'catmullrom',
  0.48,
...
const routePoints = [
  new THREE.Vector3(0, 0.055, 9),
  new THREE.Vector3(3, 0.055, 8.4),
  new THREE.Vector3(5, 0.055, 6),
  new THREE.Vector3(5.45, 0.055, 2),
  new THREE.Vector3(5.45, 0.055, -4),
  new THREE.Vector3(5.45, 0.055, -10),
  new THREE.Vector3(5.4, 0.055, -15.3),
  new THREE.Vector3(4.95, 0.055, -18.5),
  new THREE.Vector3(3, 0.055, -20.5),
  new THREE.Vector3(0.65, 0.055, -21.8),
  new THREE.Vector3(-0.35, 0.055, -22.35),
  new THREE.Vector3(0, 0.055, -25.15),
];
...
function segment(material, radius, parent) { window.__bfTrace?.add(406);
  return mesh(new THREE.CylinderGeometry(radius, radius * 0.9, 1, 12), material, parent);
}

function alignSegment(object, start, end) { window.__bfTrace?.add(410);
...
const tmpTimingPoint = new THREE.Vector3();
const tmpRouteLookupPoint = new THREE.Vector3();
const cameraRoutePoint = new THREE.Vector3();
const cameraTarget = new THREE.Vector3();

function clamp01(value) { window.__bfTrace?.add(450);
  return Math.max(0, Math.min(1, value));
}

function smoothstep(value) { window.__bfTrace?.add(454);
  return value * value * (3 - 2 * value);
}

function travelProgress(time) { window.__bfTrace?.add(458);
...
function routeProgressAtLongitudinalProgress(progress) { window.__bfTrace?.add(472);
  if (progress <= 0 || progress >= 1) return progress;
  cameraRouteCurve.getPointAt(progress, tmpTimingPoint);
  let lower = 0;
  let upper = 1;
  for (let iteration = 0; iteration < 20; iteration += 1) {
    const midpoint = (lower + upper) * 0.5;
    routeCurve.getPointAt(midpoint, tmpRouteLookupPoint);
    if (tmpRouteLookupPoint.z > tmpTimingPoint.z) {
      lower = midpoint;
    } else {
      upper = midpoint;
    }
...
function updateRobot(time) { window.__bfTrace?.add(489);
  const travel = routeProgressAtLongitudinalProgress(travelProgress(time));
...
function updateCamera(time) { window.__bfTrace?.add(542);
  const p = travelProgress(time);
  cameraRouteCurve.getPointAt(p, cameraRoutePoint);
  camera.position.set(
...
function renderAt(time) { window.__bfTrace?.add(561);
...
function animate() { window.__bfTrace?.add(568);
  if (!paused) {
