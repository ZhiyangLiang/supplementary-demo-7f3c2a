...
function roundedRectShape(width, depth, radius) { window.__bfTrace?.add(39);
  const x = -width / 2;
  const y = -depth / 2;
...
function roundedPrism(width, depth, height, radius, material, bevel = 0.025) { window.__bfTrace?.add(55);
...
function planeMaterial(color, opacity = 1) { window.__bfTrace?.add(75);
...
function addKey(x, z, width = 0.335) { window.__bfTrace?.add(234);
...
const screenLayers = [];
function addScreenLayer(geometry, material, x, y, z = 0.02) { window.__bfTrace?.add(341);
  const mesh = new THREE.Mesh(geometry, material);
...
function createPchipTrack(points, zeroEndpointSlopes = false) { window.__bfTrace?.add(509);
...
function updateScene(seconds) { window.__bfTrace?.add(565);
...
function tick(now) { window.__bfTrace?.add(603);
  if (!playing) return;
...
function pause() { window.__bfTrace?.add(616);
  playing = false;
...
function seek(seconds) { window.__bfTrace?.add(625);
  pause();
  return updateScene(seconds);
}

function play() { window.__bfTrace?.add(630);
  pause();
  playing = true;
...
function getCameraState() { window.__bfTrace?.add(638);
  return {
