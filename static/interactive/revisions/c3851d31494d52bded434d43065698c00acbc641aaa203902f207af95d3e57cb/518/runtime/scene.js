...
function shadowed(object, cast = true, receive = false) { window.__bfTrace?.add(88);
  object.castShadow = cast;
  object.receiveShadow = receive;
  return object;
}

function mesh(geometry, material, cast = true, receive = false) { window.__bfTrace?.add(94);
  return shadowed(new THREE.Mesh(geometry, material), cast, receive);
}

function material(color, roughness = 0.7, metalness = 0) { window.__bfTrace?.add(98);
  return new THREE.MeshStandardMaterial({ color, roughness, metalness });
}

function smoothstep(a, b, value) { window.__bfTrace?.add(102);
  const x = THREE.MathUtils.clamp((value - a) / (b - a), 0, 1);
  return x * x * (3 - 2 * x);
}

function lerpVector(a, b, amount) { window.__bfTrace?.add(107);
  return new THREE.Vector3().lerpVectors(a, b, amount);
}

function seeded(index, salt = 0) { window.__bfTrace?.add(111);
  const value = Math.sin(index * 127.1 + salt * 311.7) * 43758.5453123;
  return value - Math.floor(value);
}

function roundedLimb(length, radius, color) { window.__bfTrace?.add(116);
...
function createTree(x, z, scale, hue = 0) { window.__bfTrace?.add(128);
...
function createCloud(x, y, z, scale) { window.__bfTrace?.add(160);
...
const legs = [];
function addLeg(x, z, upperAngle, lowerAngle) { window.__bfTrace?.add(374);
...
function eventTimeAt(time) { window.__bfTrace?.add(484);
...
function dogHeight(eventTime) { window.__bfTrace?.add(490);
  const phase = THREE.MathUtils.clamp(eventTime / 1.8, 0, 1);
  return 1.0 + 1.5 * Math.sin(Math.PI * phase);
}

function updateWorld(time) { window.__bfTrace?.add(495);
...
function updateCamera(time) { window.__bfTrace?.add(554);
...
function renderAt(time) { window.__bfTrace?.add(585);
...
function animate(now) { window.__bfTrace?.add(592);
  if (paused) return;
