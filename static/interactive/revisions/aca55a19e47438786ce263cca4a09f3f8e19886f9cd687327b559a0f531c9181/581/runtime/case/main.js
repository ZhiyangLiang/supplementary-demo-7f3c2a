...
scene.add(world);
let seed = 581;
function random() { window.__bfTrace?.add(20);
  seed = (1664525 * seed + 1013904223) >>> 0;
  return seed / 4294967296;
}
function material(color, roughness = 0.65, metalness = 0) { window.__bfTrace?.add(24);
  return new THREE.MeshStandardMaterial({ color, roughness, metalness });
}
function noisyMaterial(color, roughness, metalness, scale = 6, strength = 0.18) { window.__bfTrace?.add(27);
...
function mesh(parent, geo, mat, name, x = 0, y = 0, z = 0) { window.__bfTrace?.add(60);
...
function box(parent, name, size, pos, mat = white) { window.__bfTrace?.add(69);
  return mesh(parent, new THREE.BoxGeometry(...size), mat, name, ...pos);
}
function sphere(parent, name, pos, scale, mat, detail = 20) { window.__bfTrace?.add(72);
  const m = mesh(parent, new THREE.SphereGeometry(1, detail, 12), mat, name, ...pos);
  m.scale.set(...scale);
  return m;
}
function rod(parent, name, a, b, radius, mat, sides = 10) { window.__bfTrace?.add(77);
...
function tube(parent, name, points, radius, mat, closed = false) { window.__bfTrace?.add(84);
...
function labelTexture(text, bg, fg, w = 256, h = 96) { window.__bfTrace?.add(87);
...
function sign(parent, name, text, pos, size, bg, fg) { window.__bfTrace?.add(95);
...
function pedestal(name, x, z, w, h, d, allWhite = false) { window.__bfTrace?.add(160);
...
function iBeam(parent, name, pos, length, rz, ry, depth = 0.42) { window.__bfTrace?.add(168);
...
function irregular(parent, name, pos, scale, mat, k = 0.12, detail = 2) { window.__bfTrace?.add(183);
...
function painting(side, z, y, w, h, colors) { window.__bfTrace?.add(255);
...
function person(name, x, z, shirtColor, pantsColor, yaw, pose) { window.__bfTrace?.add(269);
...
function clamp(v,a=0,b=1){ window.__bfTrace?.add(386);return Math.max(a,Math.min(b,v));}
function smooth(v){ window.__bfTrace?.add(387);v=clamp(v);return v*v*(3-2*v);}
function envelope(t){ window.__bfTrace?.add(388);
...
function yawAtDistance(route,d){ window.__bfTrace?.add(395);
  const tangent=route.curve.getTangentAt(clamp(d/route.length));
  return Math.atan2(-tangent.x,-tangent.z);
}
function poseAtDistance(route,d){ window.__bfTrace?.add(399);
...
function angleMix(a,b,t){ window.__bfTrace?.add(404);return a+Math.atan2(Math.sin(b-a),Math.cos(b-a))*t;}
function transformOffset(p,yaw,x,z){ window.__bfTrace?.add(405);
  return new THREE.Vector3(p.x+Math.cos(yaw)*x+Math.sin(yaw)*z,0,p.z-Math.sin(yaw)*x+Math.cos(yaw)*z);
}
function setSegment(obj,a,b,width,isBox=false){ window.__bfTrace?.add(408);
...
function updateRobot(t){ window.__bfTrace?.add(413);
...
function render(){ window.__bfTrace?.add(480);
...
function serializeDefinition(root){ window.__bfTrace?.add(492);
...
function getCameraState(){ window.__bfTrace?.add(520);return {position:camera.position.toArray(),quaternion:camera.quaternion.toArray(),fov:camera.fov};}
function getAgentDefinitionState(){ window.__bfTrace?.add(521);return {definition:agentDefinition,scale:[1,1,1],gait:gaitConfig};}
function getInvariantState(){ window.__bfTrace?.add(522);
...
function seek(seconds){ window.__bfTrace?.add(527);
