...
function material(name, color, roughness = 0.65, metalness = 0) { window.__bfTrace?.add(27);
  const m = new THREE.MeshStandardMaterial({ color, roughness, metalness });
  m.name = name;
  return m;
}
function proceduralTexture(kind) { window.__bfTrace?.add(32);
...
function mesh(geometry, mat, pos, parent = room, name = '') { window.__bfTrace?.add(97);
...
function box(w, h, d, mat, x, y, z, parent = room, name = '') { window.__bfTrace?.add(105);
  return mesh(new THREE.BoxGeometry(w, h, d), mat, v3(x, y, z), parent, name);
}
function cyl(rt, rb, h, mat, x, y, z, parent = room, segments = 32) { window.__bfTrace?.add(108);
  return mesh(new THREE.CylinderGeometry(rt, rb, h, segments), mat, v3(x, y, z), parent);
}
function ellipsoid(x, y, z, sx, sy, sz, mat, parent = room) { window.__bfTrace?.add(111);
  const m = mesh(new THREE.SphereGeometry(1, 16, 12), mat, v3(x, y, z), parent);
  m.scale.set(sx, sy, sz); return m;
}
function rod(a, b, radius, mat, parent = room, radiusB = radius) { window.__bfTrace?.add(115);
...
function tube(points, radius, mat, parent = room) { window.__bfTrace?.add(121);
  return mesh(new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points), 48, radius, 8, false), mat, null, parent);
}
function sign(text, w, h, x, y, z, options = {}) { window.__bfTrace?.add(124);
...
function elevator(x, selected) { window.__bfTrace?.add(155);
...
function leaves(instances, mat, parent = room) { window.__bfTrace?.add(211);
...
function tree(x, z, base, scale = 1, parent = room) { window.__bfTrace?.add(220);
...
function fern(x, y, z, scale = 1, parent = room) { window.__bfTrace?.add(237);
...
function pottedPlant(x, z, scale, type = 'fern') { window.__bfTrace?.add(272);
...
function routeGeometry(path) { window.__bfTrace?.add(427);
...
function progressAt(t) { window.__bfTrace?.add(445);
...
function routeSample(distance) { window.__bfTrace?.add(455);
...
function localOffset(x, z, yaw) { window.__bfTrace?.add(463); return v3(Math.cos(yaw) * x + Math.sin(yaw) * z, 0, -Math.sin(yaw) * x + Math.cos(yaw) * z); }
function placeSegment(m, a, b) { window.__bfTrace?.add(464);
...
function animateRobot(t) { window.__bfTrace?.add(468);
...
function transformState(o) { window.__bfTrace?.add(511);
...
function geometryDefinition(g) { window.__bfTrace?.add(514);
...
function materialDefinition(m) { window.__bfTrace?.add(522);
...
function hierarchyDefinition(root) { window.__bfTrace?.add(527);
...
function getCameraState() { window.__bfTrace?.add(541); return { position: camera.position.toArray(), quaternion: camera.quaternion.toArray(), fov: camera.fov }; }
function seek(seconds) { window.__bfTrace?.add(542);
