...
function rng(seed) { window.__bfTrace?.add(21);
  let s = seed >>> 0;
  return () => ((s = (1664525 * s + 1013904223) >>> 0) / 4294967296);
}
function generatedTexture(kind) { window.__bfTrace?.add(25);
...
function rod(parent, name, a, b, r, material) { window.__bfTrace?.add(99);
...
function cable(parent, name, points, material = M.rubber, radius = .007) { window.__bfTrace?.add(106);
  const curve = new THREE.CatmullRomCurve3(points.map(p => new THREE.Vector3(...p)));
  return mesh(new THREE.TubeGeometry(curve, 24, radius, 6, false), material, parent, name);
}
function shell(parent, name, x, y, z, w, h, d, material, radius = .035) { window.__bfTrace?.add(110);
...
function bolts(parent, x, y, z, dx, dy, axis = 'z') { window.__bfTrace?.add(122);
...
function drawers(parent, name, x, z, width, depth, face = 'front') { window.__bfTrace?.add(165);
...
function components(parent, x, y, z, count) { window.__bfTrace?.add(193);
...
function stool(name, x, z, scale = 1) { window.__bfTrace?.add(256);
...
function contactShadow(parent, name, width, depth) { window.__bfTrace?.add(373);
  const g = group(name, parent);
...
function actuator(parent, name, x, y, z, radius, length) { window.__bfTrace?.add(381);
...
function limb(parent, name, length, width, depth, material) { window.__bfTrace?.add(424);
  const g = group(name, parent);
...
function navAt(time) { window.__bfTrace?.add(496);
  const p = time / END;
...
function footContact(time, period, offset, stance, lateral, longitudinal = 0, lift = .08) { window.__bfTrace?.add(503);
...
function setLink(object, a, b) { window.__bfTrace?.add(519);
...
function solveLeg(a, b, l1, l2, forward) { window.__bfTrace?.add(523);
...
function supportedHipHeight(target, feet, reach, maxCrouch) { window.__bfTrace?.add(533);
  let height = target;
...
function animateHuman(time, n) { window.__bfTrace?.add(543);
...
function animateDog(time, n) { window.__bfTrace?.add(575);
...
function cameraAt(time) { window.__bfTrace?.add(602);
...
function seek(time) { window.__bfTrace?.add(611);
...
function getCameraState() { window.__bfTrace?.add(622);
...
function definition(value, cache) { window.__bfTrace?.add(626);
  if (!cache.has(value.uuid)) cache.set(value.uuid, value.toJSON());
  return cache.get(value.uuid);
}
function objectState(o, includeDefinitions) { window.__bfTrace?.add(630);
