...
function mesh(geometry, material, parent, position = [0, 0, 0], rotation = [0, 0, 0], scale = [1, 1, 1]) { window.__bfTrace?.add(88);
...
function cylinderBetween(parent, start, end, radius, material, radialSegments = 10) { window.__bfTrace?.add(99);
...
function makeWheel(parent, x, front = false) { window.__bfTrace?.add(113);
...
function makeMotorcycle() { window.__bfTrace?.add(141);
...
function eventTime(time) { window.__bfTrace?.add(451);
...
function sprayAmountForTime(physicalTime) { window.__bfTrace?.add(457);
  return smoothstep((physicalTime - 0.18) / 0.72)
    * (1 - smoothstep((physicalTime - 1.48) / 0.58));
}

function sparkAmountForTime(physicalTime) { window.__bfTrace?.add(462);
  return smoothstep((physicalTime - 0.3) / 0.56)
    * (1 - smoothstep((physicalTime - 1.22) / 0.48));
}

function updateInstances(physicalTime) { window.__bfTrace?.add(467);
...
function updateCamera(time, bikeX) { window.__bfTrace?.add(542);
...
function update(time) { window.__bfTrace?.add(559);
...
function animate() { window.__bfTrace?.add(585);
