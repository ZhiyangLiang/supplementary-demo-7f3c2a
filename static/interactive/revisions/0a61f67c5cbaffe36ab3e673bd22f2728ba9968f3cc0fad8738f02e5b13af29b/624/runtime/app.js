...
function createLoft(sections, radialSegments, material) { window.__bfTrace?.add(190);
  const positions = [];
  const indices = [];
...
function makeQuad(points, material) { window.__bfTrace?.add(236);
  const geometry = new THREE.BufferGeometry();
...
function makeLine(points, color, opacity = 1, radius = 0.014) { window.__bfTrace?.add(247);
...
function smootherStep(value) { window.__bfTrace?.add(519);
...
function sampleCameraProgress(normalizedTime) { window.__bfTrace?.add(544);
...
function sampleCamera(normalizedTime) { window.__bfTrace?.add(560);
...
function updateAtTime(seconds) { window.__bfTrace?.add(576);
...
function animate(now) { window.__bfTrace?.add(605);
  if (!playing) return;
