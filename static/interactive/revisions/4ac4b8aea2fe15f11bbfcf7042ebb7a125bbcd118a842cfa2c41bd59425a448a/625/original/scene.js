...
function makeLoft(sections, radialSegments, loftMaterial) {
  const vertices = [];
  const normals = [];
  const uvs = [];
  const indices = [];
...
function roundedBox(width, height, depth, radius, boxMaterial) {
...
function makeTube(points, radius, tubeMaterial, tubularSegments = 32) {
...
function interpolateSections(sections, x) {
  let index = 0;
