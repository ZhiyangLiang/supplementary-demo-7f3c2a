...
function mesh(name, geometry, material, parent = sharedRoot) { window.__bfTrace?.add(62);
...
function box(name, dimensions, position, material, parent = sharedRoot) { window.__bfTrace?.add(71);
...
function frontWall(side) { window.__bfTrace?.add(88);
...
function painting(name, width, height, kind, seed, position, parent = sharedRoot) { window.__bfTrace?.add(100);
  const group = new THREE.Group();
  group.name = name;
...
function trackLine(name, from, to) { window.__bfTrace?.add(202);
...
function trackLamp(name, position, target, power = 50, cone = .52) { window.__bfTrace?.add(208);
  const group = new THREE.Group();
  group.name = name;
  group.position.set(...position);
  sharedRoot.add(group);
...
function cameraProgress(t) { window.__bfTrace?.add(283);
...
function updateCamera(t) { window.__bfTrace?.add(300);
...
function render() { window.__bfTrace?.add(316);
...
function seek(seconds) { window.__bfTrace?.add(331);
...
function setVariant(id) { window.__bfTrace?.add(339);
...
function getCameraState() { window.__bfTrace?.add(347);
  return { position: camera.position.toArray(), quaternion: camera.quaternion.toArray(), fov: camera.fov };
}

function serializeNode(object, definitions) { window.__bfTrace?.add(351);
  const state = {
...
function serializedTree(root) { window.__bfTrace?.add(402);
