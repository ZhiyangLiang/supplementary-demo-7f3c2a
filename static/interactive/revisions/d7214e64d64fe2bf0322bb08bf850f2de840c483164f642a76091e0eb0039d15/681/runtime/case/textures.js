...
function hash(x, y, seed) { window.__bfTrace?.add(11);
  let n = Math.imul(x, 374761393) + Math.imul(y, 668265263) + seed * 1013;
  n = Math.imul(n ^ (n >>> 13), 1274126177);
  return ((n ^ (n >>> 16)) >>> 0) / 4294967295;
}

function noise(x, y, seed, period = 0) { window.__bfTrace?.add(17);
...
function finish(canvas, name, recipe) { window.__bfTrace?.add(28);
