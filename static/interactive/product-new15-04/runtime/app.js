...
function mesh(g,m,x=0,y=0,z=0){ window.__bfTrace?.add(13);const o=new THREE.Mesh(g,m);o.position.set(x,y,z);o.castShadow=true;o.receiveShadow=true;scene.add(o);return o;}
function rounded(w,h,d,r,m,x,y,z){ window.__bfTrace?.add(14);
...
function cyl(r,len,m,x,y,z){ window.__bfTrace?.add(18);const o=mesh(new THREE.CylinderGeometry(r,r,len,80),m,x,y,z);o.rotation.x=Math.PI/2;return o;}
...
function area(x,y,z,p,color,size){ window.__bfTrace?.add(40);const l=new THREE.DirectionalLight(color,p);l.position.set(x,y,z);l.castShadow=true;l.shadow.mapSize.set(1024,1024);l.shadow.camera.left=-5;l.shadow.camera.right=5;l.shadow.camera.top=5;l.shadow.camera.bottom=-5;l.shadow.normalBias=.025;l.shadow.bias=-.0002;l.shadow.radius=size;l.shadow.blurSamples=12;scene.add(l);}
...
function smooth(a,b,x){ window.__bfTrace?.add(51);const t=THREE.MathUtils.clamp((x-a)/(b-a),0,1);return t*t*(3-2*t);}
function seek(seconds){ window.__bfTrace?.add(52);
