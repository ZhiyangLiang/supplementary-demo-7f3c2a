...
function cylinderBetween(a, b, radius, material, parent = scene, radialSegments = 8) {
...
function dampedJolt(t, start, strength, frequency, decay) {
...
function addCedar(x, z, radius, height, index) {
...
const spiritHeads = [];
function addSmallSpirit(x, y, z, scale, facing, perchTilt = 0) {
  const group = new THREE.Group();
...
const animalGroups = [];
function addDeer(x, z, scale, material, angle) {
  const group = new THREE.Group();
