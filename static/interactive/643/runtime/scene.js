...
function box(name, size, position, material, cast = true, receive = true) { window.__bfTrace?.add(126);
...
function roundedBox(name, size, position, radius, material) { window.__bfTrace?.add(162);
...
function addLine(start, end, material, radius = 0.025) { window.__bfTrace?.add(200);
...
function smootherstep(value) { window.__bfTrace?.add(499);
  const x = THREE.MathUtils.clamp(value, 0, 1);
  return x * x * x * (x * (x * 6 - 15) + 10);
}

function updateAtTime(seconds) { window.__bfTrace?.add(504);
