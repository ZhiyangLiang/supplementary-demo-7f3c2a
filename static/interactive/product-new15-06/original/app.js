...
function area(x,y,z,power,color,size){const l=new THREE.DirectionalLight(color,power);l.position.set(x,y,z);l.castShadow=true;l.shadow.mapSize.set(2048,2048);l.shadow.camera.left=-7;l.shadow.camera.right=7;l.shadow.camera.top=7;l.shadow.camera.bottom=-7;l.shadow.normalBias=.015;l.shadow.bias=-.00015;l.shadow.radius=size;scene.add(l);}
...
function cyl(r,h,m,x,y,z,parent=scene,r2=r){return mesh(new THREE.CylinderGeometry(r,r2,h,128),m,x,y,z,parent);}
function box(w,h,d,m,x,y,z,parent=scene){return mesh(new THREE.BoxGeometry(w,h,d),m,x,y,z,parent);}
function rounded(w,d,h,r,m,x,y,z){
...
function rod(a,b,r,m){const av=new THREE.Vector3(...a),bv=new THREE.Vector3(...b),delta=bv.clone().sub(av);const o=mesh(new THREE.CylinderGeometry(r,r,delta.length(),24),m,...av.clone().add(bv).multiplyScalar(.5).toArray());o.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),delta.normalize());return o;}
...
function tick(now){if(!paused)update(((now-start)/1000)%(124/24));requestAnimationFrame(tick);}update(0);requestAnimationFrame(tick);
