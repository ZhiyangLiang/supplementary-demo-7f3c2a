...
function box(name, size, position, material, parent = scene, cast = true, receive = true) { window.__bfTrace?.add(64);
...
function cylinder(name, radius, depth, position, material, parent = scene, radialSegments = 20) { window.__bfTrace?.add(74);
...
function beamBetween(name, a, b, radius, material, parent = scene) { window.__bfTrace?.add(87);
...
function addHazardStrip(parent, x, y, z, width, horizontal = true) { window.__bfTrace?.add(96);
  const stripeCount = 9;
...
function makePalletStack(x, z, rotation, cartons) { window.__bfTrace?.add(114);
...
function makeDrum(x, z, colorMaterial) { window.__bfTrace?.add(144);
  const group = new THREE.Group();
  group.position.set(x, 0, z);
  scene.add(group);
...
// Half-raised rolling shutter and its hardware.
const door = new THREE.Group();
door.name = 'half-raised-loading-door';
scene.add(door);
...
addHazardStrip(door, 0, shutterBottom - 0.16, -13.08, 9.8, true);
...
// Bright procedural exterior world visible through the physical opening.
...
// Lighting keeps the interior readable while making the open doorway unmistakably bright.
...
function smootherstep(value) { window.__bfTrace?.add(365);
  const t = THREE.MathUtils.clamp(value, 0, 1);
  return t * t * t * (t * (t * 6 - 15) + 10);
}

function update(time) { window.__bfTrace?.add(370);
