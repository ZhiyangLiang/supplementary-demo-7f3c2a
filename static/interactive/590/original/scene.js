...
function box(name, size, position, material, parent = scene, cast = true, receive = true) {
...
function cylinder(name, radius, depth, position, material, parent = scene, radialSegments = 20) {
...
function beamBetween(name, a, b, radius, material, parent = scene) {
...
function addHazardStrip(parent, x, y, z, width, horizontal = true) {
  const stripeCount = 9;
...
function makePalletStack(x, z, rotation, cartons) {
...
function makeDrum(x, z, colorMaterial) {
  const group = new THREE.Group();
  group.position.set(x, 0, z);
  scene.add(group);
