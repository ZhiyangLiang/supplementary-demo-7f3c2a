...
function mesh(geometry, material, position, rotation = [0, 0, 0], parent = scene) { window.__bfTrace?.add(72);
...
function box(size, material, position, rotation, parent) { window.__bfTrace?.add(82);
  return mesh(new THREE.BoxGeometry(...size), material, position, rotation, parent);
}

function cylinder(radiusTop, radiusBottom, height, segments, material, position, rotation, parent) { window.__bfTrace?.add(86);
  return mesh(
...
function smooth01(value) { window.__bfTrace?.add(96);
  const x = THREE.MathUtils.clamp(value, 0, 1);
  return x * x * (3 - 2 * x);
}

function easeOutCubic(value) { window.__bfTrace?.add(101);
  const x = THREE.MathUtils.clamp(value, 0, 1);
  return 1 - (1 - x) ** 3;
}
...
function createArm(side) { window.__bfTrace?.add(351);
  const group = new THREE.Group();
  camera.add(group);
...
function alignSegment(object, start, end) { window.__bfTrace?.add(389);
...
function setArmPose(arm, handPosition, elbowPosition, twist = 0) { window.__bfTrace?.add(397);
...
function tankLocal(vector) { window.__bfTrace?.add(408);
...
function updateTank(time) { window.__bfTrace?.add(418);
...
function updateCamera(time) { window.__bfTrace?.add(438);
...
function updateArms(time) { window.__bfTrace?.add(488);
...
function updateEffects(time) { window.__bfTrace?.add(524);
...
function renderAt(time) { window.__bfTrace?.add(576);
...
function animate(now) { window.__bfTrace?.add(590);
  if (paused) {
    return;
  }
