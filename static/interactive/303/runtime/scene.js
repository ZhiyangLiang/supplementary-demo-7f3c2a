...
function standard(color, roughness = 0.8, metalness = 0) { window.__bfTrace?.add(39);
...
function terrainHeight(x, z) { window.__bfTrace?.add(130);
...
function createTerrain() { window.__bfTrace?.add(138);
  const xSegments = 72;
  const zSegments = 84;
...
function riverCenter(z) { window.__bfTrace?.add(172);
  return 5 + Math.sin((z + 28) * 0.055) * 10 + Math.sin((z - 10) * 0.018) * 5;
}

function createRiver() { window.__bfTrace?.add(176);
  const segments = 75;
  const vertices = [];
  const indices = [];
  const bankVertices = [[], []];
...
function jaggedRock(radius, height, color, radialSegments = 9) { window.__bfTrace?.add(232);
...
function createCliffs() { window.__bfTrace?.add(246);
...
function createTower() { window.__bfTrace?.add(271);
...
function createTrees() { window.__bfTrace?.add(338);
  const count = 250;
...
function conicalRoof(radius, height, color = materials.roof) { window.__bfTrace?.add(375);
  const roof = new THREE.Mesh(new THREE.ConeGeometry(radius, height, 8), color);
  roof.castShadow = true;
  return roof;
}

function createCastle() { window.__bfTrace?.add(381);
...
function createMountains() { window.__bfTrace?.add(510);
...
function createClouds() { window.__bfTrace?.add(534);
  const clouds = [];
...
function createHero() { window.__bfTrace?.add(561);
...
function createBirds() { window.__bfTrace?.add(737);
  const birds = [];
  for (let i = 0; i < 5; i += 1) {
...
function updateHero(time) { window.__bfTrace?.add(771);
  let x;
  let y;
  let z;
  if (time <= 1.05) {
...
function updateCamera(time) { window.__bfTrace?.add(818);
  if (time < 1.22) {
...
function updateWorld(time) { window.__bfTrace?.add(853);
  updateHero(time);
  updateCamera(time);
...
function renderAt(time) { window.__bfTrace?.add(884);
  currentTime = Math.min(DURATION, Math.max(0, Number(time) || 0));
  updateWorld(currentTime);
  renderer.render(scene, camera);
}

function animate(now) { window.__bfTrace?.add(890);
  if (paused) return;
