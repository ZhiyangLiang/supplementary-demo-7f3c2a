...
function box(name, size, position, material, parent = scene, shadow = true) {
...
function cylinder(name, radius, height, position, material, parent = scene, segments = 24) {
...
function addCeilingLight(x, z, parent = scene, intensity = 0.72) {
...
// Robotics room remains static behind the sealed doors.
...
function createRobotArm(x, z, mirror) {
...
doorRig.name = 'sealed double doors';
scene.add(doorRig);

function createDoor(side) {
...
function update(time) {
  ...
  // Reconstruct the original world state on every seek: sealed and locked.
  leftDoor.rotation.y = 0;
  rightDoor.rotation.y = 0;
  for (const material of [statusLeftMaterial, statusRightMaterial]) {
    material.color.setHex(0xff3a31);
    material.emissive.setHex(0xff1b13);
  }
