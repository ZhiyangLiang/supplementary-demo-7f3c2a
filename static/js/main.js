...
var maxConcurrentGalleryLoads = 4;
...
document.querySelectorAll('.video-gallery').forEach(function (gallery) {
  var items = Array.prototype.filter.call(gallery.children, function (child) {
    return child.classList.contains('agent-control-item');
  });
  if (!items.length) return;

  var groupSize = gallery.classList.contains('four-video-gallery') ? 4 : 2;
  gallery.classList.add('paired-video-gallery');

  for (var index = 0; index < items.length; index += groupSize) {
    var rowPlayers = items.slice(index, index + groupSize);
    var row = document.createElement('div');
    row.className = 'video-pair-row ' +
      (groupSize === 4 ? 'four-video-pair-row' : 'two-video-pair-row');
    gallery.insertBefore(row, rowPlayers[0]);
    rowPlayers.forEach(function (player) { row.appendChild(player); });

    var actions = document.createElement('div');
    actions.className = 'pair-sync-actions';
    var button = document.createElement('button');
    button.className = 'pair-sync-button';
    button.type = 'button';
    button.textContent = 'Sync in Run';
    button.setAttribute('aria-label', 'Synchronize and play this video row');
    actions.appendChild(button);
    row.appendChild(actions);

    button.addEventListener('click', function (event) {
      var syncButton = event.currentTarget;
      var currentPlayers = Array.prototype.slice.call(
        syncButton.closest('.video-pair-row').querySelectorAll('.agent-control-player')
      );
      syncButton.disabled = true;
      syncButton.textContent = 'Syncing...';

      Promise.all(currentPlayers.map(function (player) {
        return queuedPlayerControllers.get(player).load();
      })).then(function () {
        var videos = currentPlayers.map(function (player) {
          return player.querySelector('.agent-control-video');
        });
        videos.forEach(function (video) {
          video.pause();
          video.currentTime = 0;
        });
        return Promise.allSettled(videos.map(function (video) { return video.play(); }));
      }).then(function () {
        syncButton.disabled = false;
        syncButton.textContent = 'Sync in Run';
      }).catch(function (error) {
        syncButton.disabled = true;
        syncButton.textContent = 'Video unavailable';
        if (window.console) console.error(error);
      });
    });
  }
...
agentControlElements.forEach(function (player, index) {
  ...
  loader.textContent = 'Loading video...';
  loader.setAttribute('aria-label', 'Loading video');
  queueGalleryLoad(player, agentControlPlayers[index], 100000 + index);
});
